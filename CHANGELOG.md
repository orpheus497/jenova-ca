...
### Fixed
- **Grammar Loading Robustness:** Made JSON grammar loading non-fatal during startup. The system now searches multiple locations (packaged `src/jenova/cortex/json.gbnf` and `llama.cpp/grammars/json.gbnf`), handles import errors safely, and logs informational messages instead of system-level errors when grammar is unavailable. Includes a packaged fallback grammar file to work without llama.cpp repository.
