...
4.  **Reflect:** Periodically, the `CognitiveEngine` enters a reflective state. It analyzes recent conversation history to identify novel conclusions or key takeaways. This process, detailed in the `generate_insight_from_history` and `generate_assumption_from_history` methods, is the cornerstone of Jenova's learning ability. This happens every 5-7 turns.

### 3.2. The Cortex: A Graph-Based Cognitive Core
...
-   **Verification:** The `/verify` command allows the user to help the AI validate its assumptions. The AI will ask a clarifying question to confirm or deny an unverified assumption. Based on the user's response, the assumption will be moved to the `true` or `false` category.

### 3.5. Self-Optimizing Context Window
...
### 3.7. In-Cycle Fine-Tuning Framework
...
-   **Data Preparation (`prepare_data.py`):** This script gathers all the `.json` insight files from `~/.jenova-ai/users/<username>/insights/` and transforms them into a `train.jsonl` file. This file is formatted in the instruction-following format required by `llama.cpp`.
