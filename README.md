...
4.  **Reflect:** Periodically, the `CognitiveEngine` enters a reflective state. It analyzes recent conversation history to identify novel conclusions or key takeaways. This process, detailed in the `_reflect` method, is the cornerstone of Jenova's learning ability.

### 3.2. Dynamic Insight Engine

The Insight Engine is what allows Jenova to learn. During the `Reflect` phase, the AI analyzes conversations and generates a new "insight." This insight is a small, atomic piece of knowledge.

-   **Generation:** The `CognitiveEngine` prompts the LLM to find a "single, novel insight or conclusion" from the recent conversation and format it as a JSON object with `topic` and `insight` keys.
-   **Storage:** The `InsightManager` saves this JSON object to a file in the `~/.jenova-ai/insights/` directory.
-   **Retrieval:** These insights are then made available for retrieval during the `Retrieve` phase of the cognitive cycle, immediately influencing future conversations.
