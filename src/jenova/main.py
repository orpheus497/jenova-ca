...
##This script initializes all components of the cognitive architecture and starts the terminal UI
...
##Function purpose: Main entry point that initializes all components and starts the application
def main() -> None:
    ...
    try:
        ...
        if integration_config.get('enabled', True):
            integration_layer = IntegrationLayer(cortex, None, config, file_logger, cache_manager)  # memory_search will be set after initialization
        
        memory_search: MemorySearch = MemorySearch(semantic_memory, episodic_memory, procedural_memory, config, file_logger, cortex, integration_layer, llm_interface, embedding_model, cache_manager)
        insight_manager: InsightManager = InsightManager(config, ui_logger, file_logger, insights_root, llm_interface, cortex, memory_search, integration_layer)
        ...
        assumption_manager: AssumptionManager = AssumptionManager(config, ui_logger, file_logger, user_data_root, cortex, llm_interface, integration_layer)
        ...
        cognitive_engine: CognitiveEngine = CognitiveEngine(llm_interface, memory_search, insight_manager, assumption_manager, config, ui_logger, file_logger, cortex, rag_system)
        ...
        ##Block purpose: Initialize and start terminal UI
        ui: TerminalUI = TerminalUI(cognitive_engine, ui_logger)
        tools.llm_interface = llm_interface  # Pass the llm_interface to the tools that need it
        ui.run()
