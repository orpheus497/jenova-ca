...
class SettingsMenu:
    ...
    def _initialize_categories(self):
        ...
        network = SettingsCategory(
            "network",
            "Network and distributed computing settings"
        )
        network.add_setting(SettingDefinition(
            key="network.enabled",
            name="Network Mode",
        ...
        network.add_setting(SettingDefinition(
            ...
            value_type=str,
            current_value=self.config.get('network', {}).get('peer_selection', {}).get('strategy', 'load_balanced'),
            default_value='load_balanced',
            choices=['local_first', 'load_balanced', 'fastest_peer', 'parallel_voting', 'round_robin']
        ))
        ...
        llm.add_setting(SettingDefinition(
            key="llm.max_tokens",
            name="Max Tokens",
        ...
        # Memory Category
        memory = SettingsCategory(
            "memory",
            "Memory system settings"
        )
        memory.add_setting(SettingDefinition(
        ...
        memory.add_setting(SettingDefinition(
            ...
            value_type=float,
            current_value=self.config.get('memory', {}).get('similarity_threshold', 0.7),
            default_value=0.7,
            min_value=0.0,
            max_value=1.0
        ))
        ...
        # Learning Category
        learning = SettingsCategory(
            "learning",
            "Learning and personalization settings"
        )
        learning.add_setting(SettingDefinition(
            key="user.learning_mode",
            name="Learning Mode",
            description="Enable adaptive learning from interactions",
            value_type=bool,
            current_value=self.user_profile.preferences.learning_mode if self.user_profile else True,
            default_value=True
        ))
        learning.add_setting(SettingDefinition(
            ...
            value_type=bool,
            current_value=self.user_profile.preferences.proactive_suggestions if self.user_profile else True,
            default_value=True
        ))
        learning.add_setting(SettingDefinition(
            ...
            value_type=str,
            current_value=self.user_profile.preferences.response_style if self.user_profile else "balanced",
        ...
        learning.add_setting(SettingDefinition(
            ...
            value_type=str,
            current_value=self.user_profile.preferences.expertise_level if self.user_profile else "intermediate",
        ...
        # Privacy Category
        privacy = SettingsCategory(
            "privacy",
            "Privacy and data settings"
        )
        privacy.add_setting(SettingDefinition(
            key="network.resource_sharing.share_memory",
            name="Share Memory",
            description="Allow peers to search your memory (privacy-sensitive)",
            value_type=bool,
            current_value=self.config.get('network', {}).get('resource_sharing', {}).get('share_memory', False),
            default_value=False,
            requires_restart=True
        ))
        privacy.add_setting(SettingDefinition(
            ...
            value_type=bool,
            current_value=self.config.get('network', {}).get('resource_sharing', {}).get('share_llm', True),
            default_value=True
        ))
    ...
    def set_setting_value(self, key: str, value: Any, apply_immediately: bool = False) -> bool:
        """
        Set a setting value.
    ...
    def reset_to_defaults(self, category_name: Optional[str] = None):
        ...
        """
        categories = [self.categories[category_name]] if category_name else self.categories.values()

        for category in categories:
            for setting in category.list_settings():
                self._apply_change(setting.key, setting.default_value)
                self.file_logger.log_info(f"Reset {setting.key} to default: {setting.default_value}")
        ...
        for category in self.categories.values():
            for setting in category.list_settings():
                if setting.requires_restart:
                    ...
                    if current_value != setting.default_value or setting.key in self.pending_changes:
