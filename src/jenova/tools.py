...
class ToolHandler:
    ...
    def _register_tools(self):
        ...
        if self.analysis_tools_api:
            ...
            self.tools["analyze_code_metrics"] = (
                self.analysis_tools_api.analyze_code_metrics
            )
            ...
            self.tools["disambiguate_command"] = (
                self.analysis_tools_api.disambiguate_command
            )

        # Phase 13-17: Orchestration Tools (if available)
        if self.orchestration_tools_api:
            self.tools["create_task_plan"] = (
                self.orchestration_tools_api.create_task_plan
            )
            self.tools["execute_task_plan"] = (
                self.orchestration_tools_api.execute_task_plan
            )
            ...
            self.tools["run_background_task"] = (
                self.orchestration_tools_api.run_background_task
            )

        # Phase 13-17: Automation Tools (if available)
        if self.automation_tools_api:
            self.tools["create_custom_command"] = (
                self.automation_tools_api.create_custom_command
            )
    ...
    def execute_tool(self, tool_name: str, kwargs: dict) -> any:
        ...
        if tool_name == "execute_shell_command":
            ...
            whitelist = self.config.get("tools", {}).get("shell_command_whitelist", [])
        ...
        try:
            ...
            if self.file_logger:
                self.file_logger.log_error(f"Error executing tool '{tool_name}': {e}")
