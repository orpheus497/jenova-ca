...
from jenova.utils.cache import TTLCache, CacheManager
...
class MockGraph:
    ...
    def get_nodes_by_user(self, username: str) -> list[dict[str, object]]:
        """Get nodes for user."""
        return [
            {"id": f"node_{i}", "type": "insight", "content": f"Content {i}"}
            for i in range(100)
        ]
...
class TestPerformanceMonitorPerformance:
    ...
    def test_statistics_recording_performance(self, performance_monitor: PerformanceMonitor) -> None:
