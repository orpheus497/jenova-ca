...
@pytest.fixture
def scheduler_with_executor(
    default_config: SchedulerConfig, mock_executor: Mock
) -> CognitiveScheduler:
...
def test_on_turn_complete_with_executor(
    scheduler_with_executor: CognitiveScheduler, mock_executor: Mock
) -> None:
...
def test_task_execution_failure(
    ...
    verify_task = next(
        t
        for t in scheduler_with_executor._tasks
        if t.schedule.task_type == TaskType.VERIFY_ASSUMPTION
    )
...
def test_task_execution_exception(
    scheduler_with_executor: CognitiveScheduler, mock_executor: Mock
) -> None:
...
def test_acceleration_verification_threshold(
    scheduler_with_executor: CognitiveScheduler, mock_executor: Mock
) -> None:
...
def test_no_acceleration_below_threshold(
    scheduler_with_executor: CognitiveScheduler, mock_executor: Mock
) -> None:
...
def test_due_tasks_sorted_by_priority(
    scheduler_with_executor: CognitiveScheduler, mock_executor: Mock
) -> None:
...
def test_disabled_tasks_not_executed(default_config: SchedulerConfig, mock_executor: Mock) -> None:
    ...
    verify_task = next(
        t for t in scheduler._tasks if t.schedule.task_type == TaskType.VERIFY_ASSUMPTION
    )
...
def test_turns_since_last_tracking(
    scheduler_with_executor: CognitiveScheduler, mock_executor: Mock
) -> None:
...
def test_multiple_tasks_due_same_turn(
    scheduler_with_executor: CognitiveScheduler, mock_executor: Mock
) -> None:
